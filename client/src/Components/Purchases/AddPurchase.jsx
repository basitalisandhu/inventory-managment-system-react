...
const AddPurchase = () => {
  ...
  const finalizeProductPurchaing = async (paidAmount, dueAmount) => {
    ...
    console.log(payloadData, "line 99");
    ...
    try {
      ...
      setLoading(false);
      if (response.ok) {
        toast.success("Purchase products successfully added");
        navigate("/dashboard/purchase-history");
      } else {
        toast.error("Network issue failed to fetch");
      }
    } catch (error) {
  ...
  return (
    <>
      {loading && <Loading />}
      <h1 className="mt-10 text-start sm:text-3xl text-xl font-medium title-font text-gray-900">
        Add Purchase
      </h1>
